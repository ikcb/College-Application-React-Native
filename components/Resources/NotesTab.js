...
import { setNotes } from '../../reduxConfig/actions';
...
export default function NotesTab({ data, text, search }) {
    ...
    const getNotes = async () => {
        await axios.get('https://backend-clg-app.herokuapp.com/resources/notes/')
            .then(response => response.data.length > 0 ? dispatch(setNotes(response.data)) : null);
        setRefresh(false)
    }
